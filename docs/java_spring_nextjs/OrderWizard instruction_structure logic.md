...
- **Анотування фото**:
  - Простий редактор зображень для маркування проблемних місць:
    - Canvas API для малювання на зображенні
    - Інструменти: маркер, окружність, стрілка, текст
    - Кольорова палітра для позначень (червоний - важливі дефекти, жовтий - потенційні проблеми)
  - Текстовий опис для кожного фото (до 100 символів)
  - Збереження оригіналу та анотованої версії зображення

- **Галерея фото предмета**:
  - Карусель з попереднім переглядом (використання Swiper.js)
  - Режим повноекранного перегляду при натисканні
  - Відображення мініатюр усіх фото під основним зображенням
  - Синхронізація з сервером у режимі реального часу (збереження після додавання кожного фото)

- **Технічна реалізація**:
  - Збереження фото: хостинг-сервіс AWS S3 або локальне зберігання з CDN
  - API ендпоінти:
    - POST `/api/order-items/{itemId}/photos` - завантаження фото
    - DELETE `/api/order-items/{itemId}/photos/{photoId}` - видалення фото
    - PUT `/api/order-items/{itemId}/photos/{photoId}/annotations` - оновлення анотацій
  - Структура даних у БД:
    - `photo_id` - унікальний ідентифікатор
    - `item_id` - зв'язок з предметом замовлення
    - `file_path` - шлях до файлу
    - `thumbnail_path` - шлях до мініатюри
    - `annotations` - JSON з координатами позначок
    - `description` - текстовий опис фото
