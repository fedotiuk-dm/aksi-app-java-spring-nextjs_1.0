...
} from '@/features/order-wizard/model/store/store';
...
export const useItemBasicInfoForm = () => {
  ...
  const [selectedItemName, setSelectedItemName] =
    useState<PriceListItemDTO | null>(null);
  ...
  const { data: unitsOfMeasure = [], isLoading: isLoadingUnits } =
    useUnitsOfMeasure(selectedCategory?.id || null);
  ...
  const handleItemNameChange = useCallback(
    (itemId: string) => {
      if (itemId && itemNames.length > 0) {
        ...
        // Якщо у вибраного товару є одиниця виміру, автоматично встановлюємо її
        if (itemName?.unitOfMeasure) {
          console.log(
            'Автоматично встановлюємо одиницю виміру:',
            itemName.unitOfMeasure
          );
          setValue('measurementUnit', itemName.unitOfMeasure);
        }
      }
    },
    [itemNames, setSelectedItemName, setValue]
  );
  ...
  const handleSaveItem = useCallback(
    (formData: ItemBasicInfoFormValues) => {
  ...
  const isCategoryWithKg = useCallback(
    (categoryId: string) => {
      const category = categories.find((cat) => cat.id === categoryId);
      return category
        ? KILOGRAM_CATEGORIES.includes(category.code as ServiceCategory)
        : false;
    },
    [categories]
  );
