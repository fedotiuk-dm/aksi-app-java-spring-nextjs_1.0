...
import {
  itemPhotosFormSchema,
  ItemPhotosFormValues,
} from '../model/schema/item-photos.schema';
...
export function useItemPhotosForm() {
  ...
  const handleRemovePhoto = useCallback(
    (index: number) => {
      ...
      setValue(
        'photos',
        files.filter((_, i) => i !== index).map((file) => ({ file }))
      );
    },
    [files, setValue]
  );
  ...
  const onSubmit = handleSubmit(async () => {
    try {
      ...
      if (files.length > 0) {
        ...
        // Оновлюємо предмет
        if (currentItem && currentItem.id && uploadedPhotos.length > 0) {
          ...
          // Перетворюємо id в number, якщо він у форматі string
          const itemId =
      ...
      // Отримуємо стан з глобального сховища
      const {
      ...
      if (currentItem) {
        if (currentItemIndex !== null) {
          ...
          storeUpdateItem(currentItemIndex, currentItem);
        } else {
          // Додаємо новий предмет
          console.log('Додаємо новий предмет:', currentItem);
          storeAddItem(currentItem);
        }
      }

      // Незалежно від того, чи були фото, переходимо до наступного кроку
      // Переходимо до головного екрану менеджера предметів
