...
import { useActivateCustomerForCart, useUpdateCartModifiers } from '@/shared/api/generated/cart';
...
export const CustomerSection: React.FC = () => {
  ...
  const searchParams: ListCustomersParams | undefined =
    debouncedSearchQuery.length >= 2 ? { search: debouncedSearchQuery } : undefined;

  const { data: searchResults, isLoading: isSearching } = useListCustomers(searchParams, {
    query: {
  ...
  const handleCustomerSelect = async (customer: CustomerInfo | null) => {
    ...
    try {
      await activateCustomerMutation.mutateAsync({ data: { customerId: customer.id } });
      // set default cart modifiers to avoid nulls in pricing
      await updateCartModifiersMutation.mutateAsync({
        data: { urgencyType: 'NORMAL', discountType: 'NONE' },
      });
    } catch {
      // ignore
    }
  ...
  const handleCreateCustomer = handleSubmit(async (data: NewCustomerFormData) => {
    try {
      ...
      try {
        await activateCustomerMutation.mutateAsync({ data: { customerId: newCustomer.id } });
        await updateCartModifiersMutation.mutateAsync({
          data: { urgencyType: 'NORMAL', discountType: 'NONE' },
        });
      } catch {
    ...
    <Box>
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <FormControlLabel
                      control={
                        <Checkbox
                          checked={watchedContactPreferences.includes(
                            value as CreateCustomerRequestContactPreferencesItem
                          )}
                          onChange={(e) => {
      ...
      <FormControl fullWidth sx={{ mb: 2 }}>
        ...
        <Select value={selectedBranch?.id || ''} onChange={handleBranchChange}>
          ...
          {branchesData?.data?.map((branch: BranchInfo) => (
