...
export const OrderParameters = () => {
  const { sessionId, stage3Ready, setStage3Ready } = useOrderOnepageStore();
  ...
  const { data: sessionContext } = useStage3GetSessionContext(sessionId || '', {
    query: {
      enabled: !!sessionId && stage3Ready, // Завантажуємо тільки коли Stage3 готовий
      retry: false,
    },
  });
  ...
  const handleExecutionDateChange = async (date: dayjs.Dayjs | null) => {
    ...
    try {
      ...
      // Встановлюємо готовність Stage3 після успішного оновлення параметрів
      setStage3Ready(true);
    } catch (error) {
