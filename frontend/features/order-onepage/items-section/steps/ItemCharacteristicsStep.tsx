...
export const ItemCharacteristicsStep = ({
  ...
  // Отримуємо доступні матеріали з API
  const { data: availableMaterials, isLoading: materialsLoading } =
    useSubstep2GetAvailableMaterials(sessionId || '', {
      query: {
        enabled: !!sessionId,
      },
    });
  ...
  const onSubmit = async (formData: any) => {
    ...
    try {
      // Відправляємо матеріал - використовуємо назву матеріалу напряму
