...
/**
 * Функціональний інтерфейс для валідації.
 * Дозволяє композицію валідаційних правил без if-нагромаджень.
 *
...
public interface Validator<T> {
     ...
     */
    static <T, R> Validator<T> fromOptional(Function<T, Optional<R>> optionalFunction, String errorMessage) {
        return entity -> optionalFunction.apply(entity).isPresent()
            ? ValidationResult.valid()
            : ValidationResult.invalid(errorMessage);
    }
     ...
     */
    static <T> Validator<T> fromFunction(Function<T, Boolean> validationFunction, String errorMessage) {
        return entity -> Boolean.TRUE.equals(validationFunction.apply(entity))
            ? ValidationResult.valid()
            : ValidationResult.invalid(errorMessage);
    }
