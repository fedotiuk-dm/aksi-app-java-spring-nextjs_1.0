...
/**
 * Загальні функціональні правила валідації для всіх доменів.
 * Уникають дублювання коду між різними доменами.
 */
public final class CommonValidationRules {
    ...
    private static final Pattern UPPER_CASE_CODE_PATTERN = Pattern.compile(
        "^[A-Z0-9_]+$"
    );
     ...
     */
    public static <T> Validator<T> requiredUuid(java.util.function.Function<T, UUID> uuidExtractor, String fieldName) {
        return entity -> {
     ...
     */
    public static <T> Validator<T> validUpperCaseCode(java.util.function.Function<T, String> codeExtractor, String fieldName) {
        return entity -> {
            ...
            return UPPER_CASE_CODE_PATTERN.matcher(code).matches()
                ? ValidationResult.valid()
                : ValidationResult.invalid(fieldName + " повинен бути у верхньому регістрі (A-Z, 0-9, _): " + code);
        };
    }
    ...
    public static <T> Validator<T> numberInRange(
        ...
        return entity -> {
            ...
            if (value < min || value > max) {
                return ValidationResult.invalid(fieldName + " повинен бути в діапазоні " + min + "-" + max + ", отримано: " + value);
            }
    ...
    public static <T> Validator<T> stringLength(
        ...
        return entity -> {
            ...
            if (length < minLength || length > maxLength) {
                return ValidationResult.invalid(fieldName + " повинен бути довжиною " + minLength + "-" + maxLength + " символів, отримано: " + length);
            }
    ...
    /**
     * Валідація унікальності через Repository.
     * Загальний метод для будь-яких entity з exists методом.
     */
