...
/**
 * Repository для роботи з refresh токенами
...
public interface RefreshTokenRepository extends JpaRepository<RefreshTokenEntity, Long> {
     ...
     */
    @Query("SELECT rt FROM RefreshTokenEntity rt WHERE rt.token = :token AND rt.isActive = true AND rt.expiresAt > :now")
     ...
     */
    @Query("SELECT rt FROM RefreshTokenEntity rt WHERE rt.userId = :userId AND rt.isActive = true AND rt.expiresAt > :now")
     ...
     */
    @Query("SELECT COUNT(rt) > 0 FROM RefreshTokenEntity rt WHERE rt.token = :token AND rt.isActive = true AND rt.expiresAt > :now")
     ...
     */
    @Query("SELECT rt FROM RefreshTokenEntity rt WHERE rt.isActive = true AND rt.expiresAt BETWEEN :now AND :threshold")
    List<RefreshTokenEntity> findTokensExpiringBetween(@Param("now") LocalDateTime now, @Param("threshold") LocalDateTime threshold);
     ...
     */
    @Query("SELECT COUNT(rt) FROM RefreshTokenEntity rt WHERE rt.userId = :userId AND rt.isActive = true AND rt.expiresAt > :now")
    long countActiveSessionsByUserId(@Param("userId") Long userId, @Param("now") LocalDateTime now);

    /**
     * Статистика токенів по користувачах
     */
    @Query("SELECT rt.userId, COUNT(rt) FROM RefreshTokenEntity rt WHERE rt.isActive = true GROUP BY rt.userId")
