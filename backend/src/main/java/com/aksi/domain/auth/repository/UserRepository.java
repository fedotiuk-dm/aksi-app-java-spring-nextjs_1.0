 ...
 * Spring Data JPA з custom query методами
 */
@Repository
public interface UserRepository extends JpaRepository<UserEntity, Long> {
     ...
     */
    @Query("SELECT CASE WHEN COUNT(u) > 0 THEN true ELSE false END FROM UserEntity u JOIN u.roles r WHERE r = :role")
     ...
     */
    @Query("UPDATE UserEntity u SET u.lastLoginAt = :loginTime, u.failedLoginAttempts = 0 WHERE u.id = :userId")
    void updateLastLogin(@Param("userId") Long userId, @Param("loginTime") LocalDateTime loginTime);

    /**
     * Скидання невдалих спроб входу
     */
    @Query("UPDATE UserEntity u SET u.failedLoginAttempts = 0, u.lockedUntil = null WHERE u.id = :userId")
    void resetFailedAttempts(@Param("userId") Long userId);

    /**
     * Пошук адміністраторів системи
     */
    @Query("SELECT u FROM UserEntity u JOIN u.roles r WHERE r IN ('ADMIN', 'MANAGER') AND u.isActive = true")
