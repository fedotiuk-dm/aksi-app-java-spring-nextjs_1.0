...
/**
 * Service для бізнес-логіки автентифікації та авторизації
 * Дотримується принципів DDD SOLID
 */
@Service
...
public class AuthService {
     ...
     * API метод для LoginRequest → AuthResponse
     */
    @Transactional
    public AuthResponse authenticateUser(LoginRequest loginRequest) {
        ...
        UserEntity user = findUserByUsernameOrEmail(normalizedUsername)
        ...
        return authMapper.toAuthResponse(accessToken, refreshTokenString,
                jwtTokenService.getExpirationInSeconds(), user);
    }

    /**
     * Оновлення access токену
     * API метод для RefreshTokenRequest → AuthResponse
     */
    @Transactional
    public AuthResponse refreshAccessToken(RefreshTokenRequest refreshTokenRequest) {
        ...
        UserEntity user = findUserById(refreshToken.getUserId())
    ...
    /**
     * Вихід з системи
     * API метод для logout → LogoutResponse
     */
    @Transactional
    ...
    /**
     * Отримання поточного користувача за ID
     * API метод для getUserInfo → UserResponse
     */
     ...
     * API метод для JWT Security Filter
     */
