...
/**
 * Mapper для Entity ↔ DTO конвертації в Auth домені
 * Використовує MapStruct для автоматичної генерації
 */
...
public interface AuthMapper {
     ...
     * Головний маппінг для публічної інформації користувача
     */
    ...
    /**
     * Створення нового UserEntity тільки з username з LoginRequest
     * Інші поля встановлюються в Service
     */
    ...
    default com.aksi.api.auth.dto.UserRole toApiUserRole(UserRole domainRole) {
        return domainRole != null ?
            com.aksi.api.auth.dto.UserRole.fromValue(domainRole.toApiValue()) : null;
    }
    ...
    default UserRole fromApiUserRole(com.aksi.api.auth.dto.UserRole apiRole) {
        return apiRole != null ?
            UserRole.fromApiValue(apiRole.getValue()) : null;
    }
    ...
    default List<com.aksi.api.auth.dto.UserRole> domainRolesToApiRoles(List<UserRole> domainRoles) {
        return domainRoles != null ?
            domainRoles.stream().map(this::toApiUserRole).toList() : null;
    }
    ...
    default List<UserRole> apiRolesToDomainRoles(List<com.aksi.api.auth.dto.UserRole> apiRoles) {
        return apiRoles != null ?
            apiRoles.stream().map(this::fromApiUserRole).toList() : null;
    }
