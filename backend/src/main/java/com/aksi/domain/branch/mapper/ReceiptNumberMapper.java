...
public interface ReceiptNumberMapper {
    ...
    @Mapping(target = "generatedAt", expression = "java(convertToOffsetDateTime(java.time.LocalDateTime.now()))")
    ...
    ReceiptValidationResponse toReceiptValidationResponse(String receiptNumber, boolean isValid,
                                                        java.util.List<String> errors, String branchCode);
    ...
    ReceiptNumberParseResponse toReceiptNumberParseResponse(String receiptNumber, String branchCode,
                                                          Integer year, Integer sequenceNumber, BranchEntity branch);
    ...
    default boolean isValidReceiptNumberFormat(String receiptNumber) {
        ...
        return parts.length == 3 &&
               parts[0].matches("^[A-Z0-9]+$") &&  // Код філії
               parts[1].matches("^\\d{4}$") &&     // Рік (4 цифри)
