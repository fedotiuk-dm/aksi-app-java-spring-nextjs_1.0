...
/** Validator для бізнес-правил філій. Тільки те що неможливо зробити через Bean Validation. */
@Component
...
public class BranchValidator {
  ...
  public void validateCanAcceptOrders(BranchEntity branch) {
    if (!branch.canAcceptOrders()) {
      throw new BranchValidationException(
          String.format(
              "Філія зі статусом '%s' не може приймати замовлення", branch.getStatus().name()));
    }
    ...
    //     throw new BranchValidationException("Філія не може приймати замовлення без робочого
    // розкладу");
    // }
  }
