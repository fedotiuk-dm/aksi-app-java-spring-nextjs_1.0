...
/**
 * Сервіс для роботи з геолокацією та пошуком філій
 * Відповідальність: географічний пошук, відстані, швидкий пошук
 */
@Slf4j
@Service
@Transactional
...
public class BranchLocationService {
    ...
    public List<BranchWithDistanceResponse> findNearbyBranches(
        ...
        log.debug("Finding branches near ({}, {}) within {}km, limit: {}",
        ...
        return nearbyBranches.stream()
            .map(branch -> {
                double distance = calculateDistance(latitude, longitude,
                    branch.getCoordinates().getLatitude(), branch.getCoordinates().getLongitude());
    ...
    public double calculateDistanceBetweenBranches(UUID branch1Id, UUID branch2Id) {
        ...
        BranchEntity branch1 = branchRepository.findById(branch1Id)
            .orElseThrow(() -> new RuntimeException("Branch not found: " + branch1Id));
        BranchEntity branch2 = branchRepository.findById(branch2Id)
    ...
    public List<BranchWithDistanceResponse> getNearbyBranches(
        ...
        log.debug("API: Finding branches near ({}, {}) within {}km, limit: {}",
     ...
     */
    @Transactional(readOnly = true)
    public List<BranchEntity> findNearby(double latitude, double longitude, double radiusKm, int limit) {
    ...
    public BranchEntity findNearestBranch(double latitude, double longitude) {
        ...
        return branchRepository.findAll().stream()
            .filter(branch -> branch.hasCoordinates())
            .min((b1, b2) -> {
                double dist1 = calculateDistance(latitude, longitude, b1.getCoordinates().getLatitude(), b1.getCoordinates().getLongitude());
                double dist2 = calculateDistance(latitude, longitude, b2.getCoordinates().getLatitude(), b2.getCoordinates().getLongitude());
