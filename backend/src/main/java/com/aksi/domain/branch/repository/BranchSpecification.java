...
/**
 * JPA Specifications для композиційних запитів до BranchEntity.
 * Замінює складні @Query методи на type-safe конструкції.
 */
public class BranchSpecification {
     ...
     */
    public static Specification<BranchEntity> withinRadius(double latitude, double longitude, double radiusKm) {
        return (root, query, criteriaBuilder) -> {
            ...
            var latitudeRadians = criteriaBuilder.function("radians", Double.class, criteriaBuilder.literal(latitude));
            var longitudeRadians = criteriaBuilder.function("radians", Double.class, criteriaBuilder.literal(longitude));
            var branchLatRadians = criteriaBuilder.function("radians", Double.class, root.get("coordinates").get("latitude"));
            var branchLngRadians = criteriaBuilder.function("radians", Double.class, root.get("coordinates").get("longitude"));
     ...
     */
    public static Specification<BranchEntity> searchWithFilters(String query, BranchStatus status, String city) {
        return Specification.where(containsText(query))
     ...
     */
    public static Specification<BranchEntity> nearbyWithFilters(double latitude, double longitude,
                                                               double radiusKm, BranchStatus status) {
        return Specification.where(withinRadius(latitude, longitude, radiusKm))
            .and(hasStatus(status));
    }
