...
public class WorkingScheduleException extends RuntimeException {
    ...
    public static WorkingScheduleException notFound(UUID branchId) {
        return new WorkingScheduleException(String.format("Робочий розклад для філії '%s' не знайдено", branchId));
    }

    public static WorkingScheduleException invalidWorkingHours(DayOfWeek dayOfWeek, LocalTime openTime, LocalTime closeTime) {
        return new WorkingScheduleException(
            String.format("Невалідні робочі години для %s: %s - %s", dayOfWeek, openTime, closeTime));
    }

    public static WorkingScheduleException branchClosed(UUID branchId, LocalDate date, LocalTime time) {
        return new WorkingScheduleException(
