...
/**
 * Сервіс для управління генерацією номерів квитанцій
 * Відповідальність: використання API DTO через ReceiptNumberMapper
 */
@Slf4j
@Service
@Transactional
...
public class ReceiptNumberService {
    ...
    public ReceiptValidationResponse validateReceiptNumber(ValidateReceiptNumberRequest request) {
        ...
        return receiptNumberMapper.toReceiptValidationResponse(receiptNumber, isValid, errors, branchCode);
    }
     ...
     */
    public List<ReceiptNumberResponse> generateMultipleReceiptNumbers(GenerateReceiptNumberRequest request, int count) {
        log.debug("Generating {} receipt numbers for branch: {}", count, request.getBranchId());

        if (count <= 0 || count > 100) {
            throw new IllegalArgumentException("Invalid batch size: " + count + ". Must be between 1 and 100");
        }
