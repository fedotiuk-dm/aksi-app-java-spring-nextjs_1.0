 ...
 */
@Repository
public interface DigitalSignatureRepository extends JpaRepository<DigitalSignatureEntity, Long>, JpaSpecificationExecutor<DigitalSignatureEntity> {
    ...
    @Query("SELECT COUNT(ds) FROM DigitalSignatureEntity ds WHERE ds.type = :type AND ds.signedAt BETWEEN :from AND :to")
    long countByTypeAndSignedAtBetween(@Param("type") SignatureType type,
                                      @Param("from") LocalDateTime from,
                                      @Param("to") LocalDateTime to);

    // Maintenance queries
    @Modifying
    @Query("DELETE FROM DigitalSignatureEntity ds WHERE ds.isValid = false AND ds.signedAt < :cleanupDate")
