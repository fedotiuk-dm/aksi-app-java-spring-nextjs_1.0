...
/**
 * Service для Document Domain з API методами (DTO↔DTO) та Entity методами
 * Базова версія за еталонним принципом з AuthService
 */
@Service
@Transactional
...
public class DocumentService {
    ...
    public DocumentPageResponse getDocuments(Integer page, Integer size, String sort) {
        ...
        List<DocumentResponse> documents = documentMapper.toDocumentResponseList(
            documentsPage.getContent());
     ...
     */
    public DocumentResponse updateDocumentStatus(UUID documentId, UpdateDocumentStatusRequest request) {
        DocumentEntity entity = findEntityByIdOrThrow(documentMapper.uuidToLong(documentId));
     ...
     */
        ...
        if (entity.getStatus() != DocumentStatus.DRAFT) {
            throw new DocumentValidationException(
                "Тільки документи зі статусом DRAFT можуть бути видалені. Поточний статус: " + entity.getStatus());
        }
    ...
    public ReceiptResponse getReceiptByOrderId(UUID orderId) {
        ReceiptEntity entity = receiptRepository.findByOrderId(orderId)
            .orElseThrow(() -> new ReceiptNotFoundException("Receipt not found for orderId: " + orderId));
        return documentMapper.toReceiptResponse(entity);
    }
    ...
    public DigitalSignatureResponse getDigitalSignatureById(UUID signatureId) {
        DigitalSignatureEntity entity = findDigitalSignatureEntityByIdOrThrow(
            documentMapper.uuidToLong(signatureId));
    ...
    /**
     * Обробити дані підпису та створити URL зображення
     * TODO: Реалізувати збереження зображення підпису у файловій системі або хмарному сховищі
     */
