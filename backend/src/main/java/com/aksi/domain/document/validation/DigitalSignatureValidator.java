...
public class DigitalSignatureValidator {
    ...
    private void validateDuplicateSignature(DigitalSignatureEntity signature) {
        if (signature.getDocumentId() != null && signature.getType() != null) {
            boolean exists = signatureRepository.findByDocumentIdAndType(
                signature.getDocumentId(), signature.getType()).isPresent();
        ...
        if (signature.getReceipt() != null && signature.getType() != null) {
            boolean exists = signatureRepository.findByReceiptAndType(
                signature.getReceipt(), signature.getType()).isPresent();
     ...
     */
    public void validateCanSignDocument(UUID documentId, SignatureType signatureType, String signerName) {
     ...
     */
    public void validateCanSignReceipt(ReceiptEntity receipt, SignatureType signatureType, String signerName) {
        // Перевірка існування підпису
        ...
        if (!receipt.hasPdfDocument()) {
            throw new SignatureValidationException("Квитанція не готова до підпису - відсутній PDF документ");
        }
    }
