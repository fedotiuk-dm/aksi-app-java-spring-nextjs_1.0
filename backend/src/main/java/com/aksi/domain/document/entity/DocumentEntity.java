...
/**
 * JPA Entity для документів з business методами.
 * Базується на OpenAPI схемі: DocumentResponse
 */
@Entity
...
public class DocumentEntity extends BaseEntity {
    ...
    public boolean canBeDownloaded() {
        return type != null && type.canBeDownloaded() &&
               filePath != null && !filePath.trim().isEmpty();
    }

    public boolean canBeSigned() {
        return status != null && status.canBeSignated() &&
               type != null && type.requiresSignature();
    }

    public boolean canBePrinted() {
        return status != null && status.canBePrinted() &&
               filePath != null && !filePath.trim().isEmpty();
    }
    ...
    public void markAsSigned() {
        if (!canBeSigned()) {
            throw new IllegalStateException("Документ не може бути підписаний у поточному стані: " + status);
        }
    ...
    public void markAsPrinted() {
        if (!canBePrinted()) {
            throw new IllegalStateException("Документ не може бути роздрукований у поточному стані: " + status);
        }
    ...
    public void markAsArchived() {
        if (!canBeArchived()) {
            throw new IllegalStateException("Документ не може бути заархівований у поточному стані: " + status);
        }
