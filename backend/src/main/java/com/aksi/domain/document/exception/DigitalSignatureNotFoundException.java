...
public class DigitalSignatureNotFoundException extends RuntimeException {
    ...
    public static DigitalSignatureNotFoundException byDocumentIdAndType(UUID documentId, SignatureType type) {
        return new DigitalSignatureNotFoundException(
            "Підпис типу " + type + " для документа " + documentId + " не знайдений");
    }

    public static DigitalSignatureNotFoundException byReceiptAndType(String receiptNumber, SignatureType type) {
        return new DigitalSignatureNotFoundException(
