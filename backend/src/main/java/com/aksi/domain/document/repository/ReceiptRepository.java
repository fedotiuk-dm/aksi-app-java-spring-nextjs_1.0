...
public interface ReceiptRepository extends JpaRepository<ReceiptEntity, Long>, JpaSpecificationExecutor<ReceiptEntity> {
    ...
    @Query("SELECT r FROM ReceiptEntity r WHERE r.isPrinted = true AND r.printedAt >= :from AND r.printedAt <= :to")
    List<ReceiptEntity> findPrintedBetween(@Param("from") LocalDateTime from, @Param("to") LocalDateTime to);
    ...
    @Query("SELECT r FROM ReceiptEntity r JOIN r.signatures s WHERE s.type IN ('CLIENT_HANDOVER', 'CLIENT_PICKUP') " +
    ...
    List<ReceiptEntity> findGeneratedBetween(@Param("from") LocalDateTime from, @Param("to") LocalDateTime to);

    @Query("SELECT r FROM ReceiptEntity r WHERE r.generatedBy = :generatedBy AND r.generatedAt >= :from")
    List<ReceiptEntity> findGeneratedByUserSince(@Param("generatedBy") String generatedBy,
    ...
    @Query("SELECT r FROM ReceiptEntity r WHERE r.generatedAt >= :from AND r.generatedAt <= :to")
    Page<ReceiptEntity> findGeneratedBetween(@Param("from") LocalDateTime from,
    ...
    @Query("SELECT COUNT(r) FROM ReceiptEntity r WHERE r.generatedAt >= :from AND r.generatedAt <= :to")
    ...
    Page<ReceiptEntity> searchReceipts(@Param("receiptNumber") String receiptNumber,
