...
/**
 * Спрощений validator для фотографій предметів з функціональним підходом.
...
public class ItemPhotoValidator {
    ...
    /**
     * Валідація для оновлення існуючого фото.
     * Композиція правил з перевіркою метаданих.
     */
    ...
    /**
     * Валідація для видалення фото.
     * Перевіряє чи не останнє фото для предмета.
     */
    public void validateForDelete(UUID photoUuid) {
        var photo = itemPhotoRepository.findByUuid(photoUuid)
            .orElseThrow(() -> ItemValidationException.businessRuleViolation(
