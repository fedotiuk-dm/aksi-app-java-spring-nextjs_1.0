 ...
 *
 * SMART ENGINE:
 * - 90% випадків: SimpleCalculator (% та фіксовані суми)
 * - 10% випадків: JexlCalculator (складні JEXL правила)
 */
@Service
@Transactional
...
public class ItemCalculationService {
    ...
    public ItemCalculationResponse calculateItemPrice(ItemCalculationRequest request) {
        ...
        try {
            ...
            return convertToItemCalculationResponse(request, priceListItem, result, urgencyCharge, finalPrice);

        } catch (Exception e) {
            log.error("Error calculating item price for {}: {}", request.getPriceListItemId(), e.getMessage());
    ...
    public OrderSummaryResponse calculateOrderSummary(CalculateOrderSummaryRequest request) {
        ...
        List<ItemCalculationResponse> itemCalculations = request.getItems().stream()
            .map(this::calculateItemPrice)
            .toList();

        // Розрахувати загальну суму предметів
        Double itemsTotal = itemCalculations.stream()
            .mapToDouble(ItemCalculationResponse::getFinalPrice)
            .sum();
    ...
    private CalculationResult calculateSmart(Double basePrice, List<PriceModifierEntity> modifiers) {
        ...
        if (!jexlModifiers.isEmpty()) {
            CalculationResult jexlResult = jexlCalculator.calculate(result.getFinalPrice(), jexlModifiers);
            ...
            result = CalculationResult.builder()
                .basePrice(basePrice)
    ...
    private PriceListItemEntity findPriceListItem(UUID priceListItemId) {
        return priceListItemRepository.findByUuid(priceListItemId)
    ...
    private ItemCalculationResponse convertToItemCalculationResponse(ItemCalculationRequest request,
     ...
     */
    private Double getActualBasePriceForResponse(PriceListItemEntity priceListItem, ItemCalculationRequest request) {
     ...
     */
    private void addColorPricingWarnings(ItemCalculationResponse response, PriceListItemEntity priceListItem, ItemCalculationRequest request) {
        if (priceListItem.hasColorSpecificPricing()) {
            ...
            if (priceListItem.getPriceBlack() != null) {
                warnings.add("Доступна спеціальна ціна для чорного кольору: " + priceListItem.getPriceBlack() + " грн");
            }

            if (priceListItem.getPriceColor() != null) {
                warnings.add("Доступна спеціальна ціна для кольорових виробів: " + priceListItem.getPriceColor() + " грн");
            }
    ...
    private Double calculateDiscountAmount(List<ItemCalculationResponse> itemCalculations,
        ...
        Double totalEligibleAmount = itemCalculations.stream()
            .mapToDouble(ItemCalculationResponse::getSubtotal)
            .sum();
     ...
     */
    private Double calculateBasePriceWithColor(PriceListItemEntity priceListItem, ItemCalculationRequest request) {
        Integer quantity = request.getQuantity();
        ...
        log.debug("Color pricing: item={}, color={}, isBlack={}, pricePerUnit={}",
            priceListItem.getName(), color, isBlack, pricePerUnit);
