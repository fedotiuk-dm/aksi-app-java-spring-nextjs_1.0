...
/**
 * Domain-specific функціональні правила валідації для Item Domain. Використовує загальні правила з
 * CommonValidationRules + специфічні для Item.
 */
public final class ItemValidationRules {

  // ==================== SERVICE CATEGORY RULES ====================

  /** Перевірка унікальності коду категорії. Використовує загальне правило uniqueField. */
  public static Validator<ServiceCategoryEntity> uniqueCode(ServiceCategoryRepository repository) {
    return CommonValidationRules.uniqueField(
        ServiceCategoryEntity::getCode, repository::existsByCode, "Код категорії");
  }

  /** Перевірка унікальності коду категорії при оновленні. */
  public static Validator<ServiceCategoryEntity> uniqueCodeForUpdate(
  ...
  public static Validator<ServiceCategoryEntity> categoryBusinessRules() {
    return Validator.from(
        category ->
            !Optional.ofNullable(category.getCode())
    ...
    return entity -> {
      ...
      return depth >= maxDepth
          ? ValidationResult.invalid(
              "Ієрархія категорій занадто глибока (максимум " + maxDepth + " рівнів)")
  ...
  public static Validator<UUID> categoryCanBeDeleted(ServiceCategoryRepository repository) {
    return categoryUuid -> {
      ...
      return childrenCount > 0
          ? ValidationResult.invalid(
              "Неможливо видалити категорію яка має дочірні категорії (" + childrenCount + ")")
          : ValidationResult.valid();
    };
  }
  ...
  public static Validator<PriceModifierEntity> uniqueModifierCode(
      PriceModifierRepository repository) {
    return CommonValidationRules.uniqueField(
        PriceModifierEntity::getCode, repository::existsByCode, "Код модифікатора");
  }

  /** Перевірка унікальності коду модифікатора при оновленні. */
  public static Validator<PriceModifierEntity> uniqueModifierCodeForUpdate(
      PriceModifierRepository repository) {
    return CommonValidationRules.uniqueFieldForUpdate(
  ...
  public static Validator<PriceModifierEntity> validApplicableCategories(
    ...
    return entity -> {
      ...
      if (categories.size() > 50) {
        return ValidationResult.invalid(
            "Модифікатор не може застосовуватися до більше ніж 50 категорій");
      }
  ...
  public static Validator<PriceListItemEntity> uniqueCatalogNumber(
      PriceListItemRepository repository) {
    return entity ->
        repository.existsByCatalogNumber(entity.getCatalogNumber())
            ? ValidationResult.invalid(
  ...
  public static Validator<ItemPhotoEntity> validFileSize() {
    return Validator.from(
        photo -> photo.hasValidFileSize(), "Розмір файлу перевищує максимально дозволений");
  }
