...
/**
 * Спрощений validator для предметів прайс-листа з функціональним підходом. Замінює безкінечні if-и
...
public class PriceListItemValidator {
  ...
  /** Валідація для створення нового предмета. Функціональна композиція замість if-нагромаджень. */
  public void validateForCreate(PriceListItemEntity item) {
    var result =
        ItemValidationRules.uniqueCatalogNumber(priceListItemRepository)
  ...
  /**
   * Валідація для оновлення існуючого предмета. Композиція правил з перевіркою унікальності для
   * update.
   */
  public void validateForUpdate(PriceListItemEntity item) {
    var result =
        ItemValidationRules.uniqueCatalogNumberForUpdate(priceListItemRepository)
            .and(ItemValidationRules.validCategory(serviceCategoryRepository))
            .and(ItemValidationRules.validPricing())
            .validate(item);
  ...
  /**
   * Валідація для видалення предмета. Перевіряє використання в активних замовленнях (коли буде
   * Order domain).
   */
  public void validateForDelete(UUID itemUuid) {
    ...
    // var result =
    // ItemValidationRules.itemNotUsedInActiveOrders(orderRepository).validate(itemUuid);
    // throwIfInvalid(result);
  }
