...
/**
 * Спрощений validator для модифікаторів цін з функціональним підходом. Замінює безкінечні if-и на
...
public class PriceModifierValidator {
  ...
  /**
   * Валідація для створення нового модифікатора. Функціональна композиція замість if-нагромаджень.
   */
  public void validateForCreate(PriceModifierEntity modifier) {
    var result =
        ItemValidationRules.uniqueModifierCode(priceModifierRepository)
  ...
  /**
   * Валідація для оновлення існуючого модифікатора. Композиція правил з перевіркою унікальності для
   * update.
   */
  public void validateForUpdate(PriceModifierEntity modifier) {
    var result =
        ItemValidationRules.uniqueModifierCodeForUpdate(priceModifierRepository)
            .and(ItemValidationRules.validPercentageValue())
            .and(ItemValidationRules.validFixedAmountValue())
            .and(ItemValidationRules.validApplicableCategories(serviceCategoryRepository))
            .validate(modifier);
  ...
  /**
   * Валідація для видалення модифікатора. Поки що тільки заглушка для майбутньої логіки з Order
   * domain.
   */
  public void validateForDelete(UUID modifierUuid) {
    ...
    // var result =
    // ItemValidationRules.modifierNotUsedInActiveOrders(orderRepository).validate(modifierUuid);
    // throwIfInvalid(result);
  }
