...
public class PriceListItemService {
     ...
     */
    @Transactional(readOnly = true)
    public Page<PriceListItemResponse> getPriceListItemsByCategory(UUID categoryUuid, Pageable pageable) {
     ...
     */
    @Transactional(readOnly = true)
    public List<PriceListItemResponse> searchPriceListItems(String query, UUID categoryId, Integer limit) {
     ...
     */
    @Transactional(readOnly = true)
    public com.aksi.api.item.dto.PriceListPageResponse getPriceListWithPagination(Integer page, Integer size, String sort,
                                                                                 UUID categoryId, String search, Boolean active) {
        var pageable = PageRequest.of(page != null ? page : 0,
                                    size != null ? size : 20,
    ...
    private PriceListItemEntity findByUuid(UUID uuid) {
        return repository.findByUuid(uuid)
            .orElseThrow(() -> new PriceListItemNotFoundException(uuid));
    }
    ...
    private void validateUniqueCatalogNumber(Integer catalogNumber) {
        if (repository.existsByCatalogNumber(catalogNumber)) {
            throw new PriceListItemAlreadyExistsException("Позиція з каталоговим номером " + catalogNumber + " вже існує");
        }
    }
    ...
    private PriceListItemResponse enrichAndMapToResponse(PriceListItemEntity entity) {
        ...
        categoryRepository.findByUuid(entity.getCategoryId())
            .ifPresent(category -> {
                response.setCategoryInfo(new com.aksi.api.item.dto.ServiceCategorySummary()
                    .id(category.getUuid())
