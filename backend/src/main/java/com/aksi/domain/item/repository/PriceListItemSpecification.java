...
/**
 * JPA Specifications для PriceListItemEntity.
...
public class PriceListItemSpecification {
     ...
     */
    public static Specification<PriceListItemEntity> basePriceBetween(Double minPrice, Double maxPrice) {
        return (root, query, criteriaBuilder) -> {
    ...
    public static Specification<PriceListItemEntity> quickSearch(UUID categoryId, String searchTerm) {
        return isActive()
                ...
                .and(nameContains(searchTerm));
    }

    /**
     * Предмети з каталоговим номером в діапазоні
     */
    public static Specification<PriceListItemEntity> catalogNumberBetween(Integer minNumber, Integer maxNumber) {
        return (root, query, criteriaBuilder) -> {
