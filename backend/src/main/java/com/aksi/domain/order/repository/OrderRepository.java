...
public interface OrderRepository extends JpaRepository<OrderEntity, UUID> {
    ...
    List<OrderEntity> findActiveOrdersByClientId(@Param("clientId") UUID clientId,
    ...
    Page<OrderEntity> findByCreatedAtBetween(@Param("startDate") LocalDateTime startDate,
     ...
     */
    @Query("SELECT o FROM OrderEntity o WHERE o.executionDate < :currentDate AND o.status != :completedStatus")
     ...
     */
    @Query("SELECT o FROM OrderEntity o WHERE o.urgency IN (:urgentTypes) AND o.status IN (:activeStatuses)")
    List<OrderEntity> findUrgentOrders(@Param("urgentTypes") List<String> urgentTypes,
    ...
    Page<OrderEntity> searchOrders(@Param("receiptNumber") String receiptNumber,
     ...
     */
    @Query("SELECT COUNT(o) FROM OrderEntity o WHERE o.branchId = :branchId AND o.createdAt BETWEEN :startDate AND :endDate")
    long countByBranchIdAndPeriod(@Param("branchId") UUID branchId,
     ...
     */
    @Query("SELECT o.clientId, COUNT(o) as orderCount FROM OrderEntity o " +
           "WHERE o.createdAt BETWEEN :startDate AND :endDate " +
           "GROUP BY o.clientId ORDER BY orderCount DESC")
    List<Object[]> findTopClientsByOrderCount(@Param("startDate") LocalDateTime startDate,
                                             @Param("endDate") LocalDateTime endDate,
                                             Pageable pageable);
     ...
     */
    @Query("SELECT DISTINCT o FROM OrderEntity o JOIN o.items i JOIN i.defects d " +
           "WHERE d IN (:criticalDefects) AND o.status IN (:activeStatuses)")
    List<OrderEntity> findOrdersWithCriticalDefects(@Param("criticalDefects") List<String> criticalDefects,
                                                    @Param("activeStatuses") List<OrderStatus> activeStatuses);

    /**
     * Знаходить замовлення за діапазоном сум
     */
    @Query("SELECT o FROM OrderEntity o WHERE o.calculation.totalAmount BETWEEN :minAmount AND :maxAmount")
    Page<OrderEntity> findByTotalAmountBetween(@Param("minAmount") java.math.BigDecimal minAmount,
     ...
     */
    @Query("SELECT o.status, COUNT(o), SUM(o.calculation.totalAmount) FROM OrderEntity o " +
           "WHERE o.createdAt BETWEEN :startDate AND :endDate " +
           "GROUP BY o.status")
