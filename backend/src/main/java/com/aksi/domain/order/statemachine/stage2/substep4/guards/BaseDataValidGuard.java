...
/**
 * Guard для перевірки наявності базових даних для розрахунку ціни.
 */
@Component
public class BaseDataValidGuard implements Guard<PriceDiscountState, PriceDiscountEvent> {

    private final PriceDiscountCoordinationService coordinationService;

    public BaseDataValidGuard(PriceDiscountCoordinationService coordinationService) {
    ...
    public boolean evaluate(StateContext<PriceDiscountState, PriceDiscountEvent> context) {
        try {
            ...
            return coordinationService.canCalculateBasePrice(UUID.fromString(sessionId.toString()));
        } catch (Exception e) {
            return false;
        }
    }
