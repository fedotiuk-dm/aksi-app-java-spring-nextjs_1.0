...
/**
 * Guard для перевірки готовності підетапу 3.1 - Параметри виконання.
 * Перевіряє чи можна переходити від налаштування параметрів виконання до наступного підетапу.
 *
 ...
 */
@Component
public class ExecutionParamsReadyGuard implements Guard<Stage3State, Stage3Event> {

    private final Stage3CoordinationService coordinationService;

    public ExecutionParamsReadyGuard(Stage3CoordinationService coordinationService) {
    ...
    public boolean evaluate(StateContext<Stage3State, Stage3Event> context) {
        ...
        return coordinationService.isExecutionParamsReady(sessionId);
    }
