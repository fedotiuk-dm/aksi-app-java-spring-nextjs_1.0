  ...
  public boolean isCompatibleWithMaterial(String material) {
    ...
    return switch (this) {
      ...
      case LEATHER_CLEANING ->
          materialLower.contains("шкіра")
              || materialLower.contains("нубук")
              || materialLower.contains("замша");
      case SHEEPSKIN -> materialLower.contains("дублянка") || materialLower.contains("овча");
      case FUR ->
          materialLower.contains("хутро")
              || materialLower.contains("норка")
              || materialLower.contains("лисиця");
