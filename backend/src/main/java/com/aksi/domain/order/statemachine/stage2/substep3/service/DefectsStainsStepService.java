...
public class DefectsStainsStepService {
    ...
    public DefectsStainsDTO saveDefectsStains(String wizardId, DefectsStainsDTO defectsStains) {
        ...
        try {
            ...
            itemWizardManagementService.saveStepData(wizardId, ItemWizardStep.DEFECTS_STAINS, validatedDTO);
            ...
            log.error("Помилка збереження дефектів та плям для wizardId: {}", wizardId, e);
            return validatedDTO.toBuilder()
                    .isValid(false)
    ...
    public DefectsStainsDTO loadDefectsStains(String wizardId) {
        ...
        try {
            DefectsStainsDTO loadedData = itemWizardManagementService.loadStepData(
                    wizardId, ItemWizardStep.DEFECTS_STAINS, DefectsStainsDTO.class);

            if (loadedData != null) {
                log.debug("Дефекти та плями завантажено для wizardId: {}", wizardId);
                return loadedData;
            } else {
                log.debug("Дефекти та плями не знайдені для wizardId: {}, повертаємо порожні дані", wizardId);
                return DefectsStainsDTO.builder()
            ...
            log.warn("Помилка завантаження дефектів та плям для wizardId: {}", wizardId, e);
            return DefectsStainsDTO.builder()
                    .selectedStains(Set.of())
                    .selectedDefects(Set.of())
                    .selectedRisks(Set.of())
