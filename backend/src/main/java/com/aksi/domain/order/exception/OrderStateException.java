...
public class OrderStateException extends RuntimeException {
     ...
     */
    public static OrderStateException invalidOperationForStatus(String operation, String currentStatus) {
        return new OrderStateException(
            String.format("Операція %s неможлива для замовлення в статусі %s", operation, currentStatus)
        );
    }
    ...
    public static OrderStateException clientSignatureRequired() {
        return new OrderStateException(
            "Для завершення замовлення потрібен підпис клієнта"
        );
    }
    ...
    public static OrderStateException emptyOrder() {
        return new OrderStateException(
            "Неможливо обробити замовлення без предметів"
        );
    }
}
