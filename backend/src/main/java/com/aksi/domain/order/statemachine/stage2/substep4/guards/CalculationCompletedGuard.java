...
/**
 * Guard для перевірки завершеності розрахунку ціни.
 */
@Component
public class CalculationCompletedGuard implements Guard<PriceDiscountState, PriceDiscountEvent> {

    private final PriceDiscountCoordinationService coordinationService;

    public CalculationCompletedGuard(PriceDiscountCoordinationService coordinationService) {
    ...
    public boolean evaluate(StateContext<PriceDiscountState, PriceDiscountEvent> context) {
        try {
            ...
            return coordinationService.isCalculationCompleted(UUID.fromString(sessionId.toString()));
        } catch (Exception e) {
            return false;
        }
    }
