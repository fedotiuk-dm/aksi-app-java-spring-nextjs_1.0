...
/**
 * Guard для перевірки готовності підетапу 3.3 - Конфігурація оплати.
 * Перевіряє чи можна переходити від налаштування оплати до наступного підетапу.
 *
 ...
 */
@Component
public class PaymentConfigReadyGuard implements Guard<Stage3State, Stage3Event> {

    private final Stage3CoordinationService coordinationService;

    public PaymentConfigReadyGuard(Stage3CoordinationService coordinationService) {
    ...
    public boolean evaluate(StateContext<Stage3State, Stage3Event> context) {
        ...
        return coordinationService.isPaymentConfigReady(sessionId);
    }
