...
/**
 * Guard для перевірки чи матеріал вибрано
 */
@Component
public class MaterialSelectedGuard implements Guard<ItemCharacteristicsState, ItemCharacteristicsEvent> {
    ...
    public MaterialSelectedGuard(ItemCharacteristicsCoordinationService coordinationService) {
    ...
    public boolean evaluate(StateContext<ItemCharacteristicsState, ItemCharacteristicsEvent> context) {
        try {
            ...
            if (data == null || data.getMaterial() == null || data.getMaterial().trim().isEmpty()) {
                return false;
            }

            // Валідуємо матеріал через координатор
            return coordinationService.validateMaterial(data.getMaterial()).isValid();
