...
/**
 * Guard для перевірки валідності менеджера предметів.
 * Дозволяє переходи тільки при валідному стані менеджера.
 */
@Component
public class ManagerValidGuard implements Guard<Stage2State, Stage2Event> {

    private final Stage2CoordinationService coordinationService;

    public ManagerValidGuard(final Stage2CoordinationService coordinationService) {
    ...
    public boolean evaluate(final StateContext<Stage2State, Stage2Event> context) {
        try {
            ...
            // Валідуємо менеджер через координаційний сервіс
            return coordinationService.isManagerValid(sessionId);
