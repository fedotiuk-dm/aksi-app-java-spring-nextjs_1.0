...
public interface OrderItemRepository extends JpaRepository<OrderItemEntity, UUID> {
    ...
    List<OrderItemEntity> findItemsWithCriticalDefects(@Param("criticalDefects") List<DefectType> criticalDefects);
    ...
    List<OrderItemEntity> findItemsRequiringSpecialTreatment(@Param("specialStains") List<StainType> specialStains);
    ...
    Page<OrderItemEntity> findByPriceRange(@Param("minPrice") BigDecimal minPrice,
     ...
     */
    @Query("SELECT i FROM OrderItemEntity i WHERE i.wearDegree >= :minWearDegree ORDER BY i.wearDegree DESC")
     ...
     */
    @Query("SELECT i FROM OrderItemEntity i WHERE i.defectNotes IS NOT NULL AND LENGTH(i.defectNotes) > 0")
    List<OrderItemEntity> findItemsWithDefectNotes();

    /**
     * Статистика по категоріях предметів
     */
    @Query("SELECT i.category, COUNT(i), AVG(i.finalPrice) FROM OrderItemEntity i GROUP BY i.category")
    List<Object[]> getCategoryStatistics();

    /**
     * Статистика по матеріалах
     */
    @Query("SELECT i.material, COUNT(i), AVG(i.finalPrice) FROM OrderItemEntity i " +
     ...
     */
    @Query("SELECT i FROM OrderItemEntity i WHERE i.category = :category AND i.material LIKE %:material%")
    List<OrderItemEntity> findByCategoryAndMaterial(@Param("category") ServiceCategory category,
                                                    @Param("material") String material);

    /**
     * Знаходить дублікати предметів в замовленні (за назвою, матеріалом та кольором)
     */
    @Query("SELECT i FROM OrderItemEntity i WHERE i.order.id = :orderId AND " +
     ...
     */
    @Query("SELECT DISTINCT i FROM OrderItemEntity i JOIN i.defects d WHERE d IN (:consentRequiredDefects)")
    List<OrderItemEntity> findItemsRequiringClientConsent(@Param("consentRequiredDefects") List<DefectType> consentRequiredDefects);
}
