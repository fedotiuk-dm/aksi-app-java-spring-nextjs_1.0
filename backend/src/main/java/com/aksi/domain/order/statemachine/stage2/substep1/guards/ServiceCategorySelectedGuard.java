...
/**
 * Guard для перевірки обрання категорії послуги
 */
@Component
public class ServiceCategorySelectedGuard implements Guard<ItemBasicInfoState, ItemBasicInfoEvent> {

    private final ItemBasicInfoCoordinationService coordinationService;

    public ServiceCategorySelectedGuard(ItemBasicInfoCoordinationService coordinationService) {
    ...
    public boolean evaluate(StateContext<ItemBasicInfoState, ItemBasicInfoEvent> context) {
        ...
        // Використовуємо CoordinationService для перевірки категорії
        return coordinationService.isServiceCategorySelected(sessionId);
    }
}
