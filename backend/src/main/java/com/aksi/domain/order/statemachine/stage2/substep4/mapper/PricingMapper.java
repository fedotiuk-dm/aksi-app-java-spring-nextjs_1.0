...
public interface PricingMapper {
     ...
     * ВАЖЛИВО: OrderItemDTO містить тільки дані на рівні предмета.
     * Терміновість та знижки зберігаються на рівні замовлення (OrderDTO).
     */
    @Named("fromOrderItem")
    default PricingCalculationDTO fromOrderItem(OrderItemDTO orderItem) {
        ...
        return PricingCalculationDTO.builder()
                // Базові поля з OrderItemDTO
                .baseUnitPrice(orderItem.getUnitPrice()) // OrderItemDTO містить фінальну ціну предмета
                ...
                .baseTotal(orderItem.getTotalPrice())

                // Категорія та характеристики з OrderItemDTO
                .categoryCode(orderItem.getCategory())
                .color(orderItem.getColor())

                                // ПРИМІТКА: Дефекти, плями, матеріал та інші характеристики
                // НЕ зберігаються в PricingCalculationDTO, тому що вони належать
                // до попередніх підетапів (2.2 та 2.3) та зберігаються окремо в контексті візарда

                // ПРИМІТКА: Терміновість та знижки НЕ зберігаються в OrderItemDTO,
                // вони застосовуються на рівні замовлення через OrderDTO.expediteType та OrderDTO.discountAmount
                .isExpedited(false) // Буде оновлено з OrderDTO при необхідності
    ...
    default java.util.Optional<String> extractString(Map<String, Object> data, String key) {
        ...
        return value instanceof String str ? java.util.Optional.of(str) : java.util.Optional.empty();
    }
    ...
    default java.util.Optional<Integer> extractInteger(Map<String, Object> data, String key) {
        Object value = data.get(key);
        return switch (value) {
            case Integer integer -> java.util.Optional.of(integer);
            case Number number -> java.util.Optional.of(number.intValue());
            case null, default -> java.util.Optional.empty();
        };
    }
    ...
    default java.util.Optional<BigDecimal> extractBigDecimal(Map<String, Object> data, String key) {
        ...
        return switch (value) {
            ...
            case String str -> {
                try {
                    yield java.util.Optional.of(new BigDecimal(str));
    ...
    default java.util.Optional<Boolean> extractBoolean(Map<String, Object> data, String key) {
        Object value = data.get(key);
        return value instanceof Boolean bool ? java.util.Optional.of(bool) : java.util.Optional.empty();
    }
    ...
    default Map<String, Object> toStateContextMap(PricingCalculationDTO pricing) {
        ...
        putIfNotNull(contextMap, "expediteFactor", pricing.getExpediteFactor());
