...
public class OrderWizardActionService {
    ...
    private void addExecutionParamsActions(Map<String, Boolean> actions, Map<String, Object> wizardData) {
        ...
        // Дозволяємо збереження та перехід тільки якщо є параметри
        boolean hasExecutionParams = wizardData.containsKey("executionParams") ||
                                   wizardData.containsKey("deliveryDate") ||
                                   wizardData.containsKey("urgency");

        actions.put(OrderWizardAction.SAVE_EXECUTION_PARAMS.getActionName(), hasExecutionParams);
        actions.put(OrderWizardAction.PROCEED_TO_DISCOUNTS.getActionName(), hasExecutionParams);
    }
    ...
    private void addGlobalDiscountsActions(Map<String, Boolean> actions, Map<String, Object> wizardData) {
        ...
        // Дозволяємо збереження та перехід тільки якщо знижки встановлено або пропущено
        boolean hasDiscountsOrSkipped = wizardData.containsKey("globalDiscounts") ||
                                      wizardData.containsKey("discountsSkipped") ||
                                      wizardData.containsKey("noDiscounts");

        actions.put(OrderWizardAction.SAVE_GLOBAL_DISCOUNTS.getActionName(), hasDiscountsOrSkipped);
        actions.put(OrderWizardAction.PROCEED_TO_PAYMENT.getActionName(), hasDiscountsOrSkipped);
    }
    ...
    private void addPaymentProcessingActions(Map<String, Boolean> actions, Map<String, Object> wizardData) {
        ...
        // Дозволяємо збереження та перехід тільки якщо інформація про оплату встановлена
        boolean hasPaymentInfo = wizardData.containsKey("paymentInfo") ||
                               wizardData.containsKey("paymentMethod") ||
                               wizardData.containsKey("prepayment");

        actions.put(OrderWizardAction.SAVE_PAYMENT_INFO.getActionName(), hasPaymentInfo);
        actions.put(OrderWizardAction.PROCEED_TO_ADDITIONAL.getActionName(), hasPaymentInfo);
    }
    ...
    private void addAdditionalInfoActions(Map<String, Boolean> actions, Map<String, Object> wizardData) {
        log.debug("Обробка додаткової інформації з {} елементів даних", wizardData.size());

        actions.put(OrderWizardAction.SET_ADDITIONAL_INFO.getActionName(), true);

        // Додаткова інформація опціональна, тому завжди можна зберегти та перейти далі
        actions.put(OrderWizardAction.SAVE_ADDITIONAL_INFO.getActionName(), true);
        actions.put(OrderWizardAction.PROCEED_TO_REVIEW.getActionName(), true);

        // Валідувати поточний крок можна завжди
        actions.put(OrderWizardAction.VALIDATE_CURRENT_STEP.getActionName(), true);
    }
    ...
    private void addOrderReviewActions(Map<String, Boolean> actions, Map<String, Object> wizardData) {
        ...
        actions.put(OrderWizardAction.VALIDATE_ORDER.getActionName(), true);
        actions.put(OrderWizardAction.GO_BACK.getActionName(), true);

        boolean orderValid = isOrderValid(wizardData);
        actions.put(OrderWizardAction.COMPLETE_ORDER.getActionName(), orderValid);
        actions.put(OrderWizardAction.SAVE_PROGRESS.getActionName(), orderValid);
    }
    ...
    private void addLegalAspectsActions(Map<String, Boolean> actions, Map<String, Object> wizardData) {
        log.debug("Обробка юридичних аспектів з {} елементів даних", wizardData.size());
    ...
    private void addReceiptGenerationActions(Map<String, Boolean> actions, Map<String, Object> wizardData) {
        log.debug("Обробка генерації квитанції з {} елементів даних", wizardData.size());
