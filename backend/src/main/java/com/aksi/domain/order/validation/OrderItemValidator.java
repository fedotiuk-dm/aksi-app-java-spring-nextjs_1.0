 ...
 *
 * РОЗДІЛЕННЯ ВАЛІДАЦІЇ:
 * - OpenAPI Bean Validation: @NotNull, @Size, @Min, @Max, @Pattern - форматна валідація
 * - Domain Validation: складні бізнес-правила, сумісність, стан системи
 */
@Slf4j
@Component
...
public class OrderItemValidator {
     ...
     */
    private void validateUpdatePermissions(OrderItemEntity existingItem, OrderItemEntity updatedItem) {
    ...
    private void validateCriticalDefectConsentForOrder(OrderEntity order, OrderItemEntity item) {
        boolean itemHasCriticalDefects = item.getDefects() != null &&
    ...
    private void validateCriticalDefectConsent(Set<DefectType> defects) {
        boolean hasCriticalDefects = defects.stream()
