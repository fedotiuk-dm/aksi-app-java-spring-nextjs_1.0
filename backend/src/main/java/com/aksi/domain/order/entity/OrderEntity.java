...
public class OrderEntity extends BaseEntity {
    ...
    @Embedded
    private OrderCalculationEntity calculation;
    ...
    public boolean hasCriticalDefects() {
        return items.stream()
            .anyMatch(item -> item.getDefects() != null &&
    ...
    public void addItem(OrderItemEntity item) {
        if (!canAddItems()) {
            throw new IllegalStateException("Неможливо додати предмет до замовлення в статусі: " + status);
        }
        items.add(item);
        item.setOrder(this);
        recalculateTotal();
    ...
    public void removeItem(OrderItemEntity item) {
        if (!canAddItems()) {
            throw new IllegalStateException("Неможливо видалити предмет з замовлення в статусі: " + status);
        }
        items.remove(item);
        item.setOrder(null);
        recalculateTotal();
