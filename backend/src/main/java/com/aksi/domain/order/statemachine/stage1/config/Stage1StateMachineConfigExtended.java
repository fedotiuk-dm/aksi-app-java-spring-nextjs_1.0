...
import com.aksi.domain.order.statemachine.stage1.actions.CreateNewClientAction;
...
import com.aksi.domain.order.statemachine.stage1.guards.NewClientFormReadyGuard;

/**
 * Розширена конфігурація Spring State Machine для Stage1 з підтримкою NewClientForm.
 * Інтегрує функціональність створення нового клієнта в загальний процес Stage1.
 */
@Configuration
@EnableStateMachine(name = "stage1ExtendedStateMachine")
public class Stage1StateMachineConfigExtended extends EnumStateMachineConfigurerAdapter<Stage1State, Stage1Event> {
    ...
    // Нові компоненти для NewClientForm
    private final CreateNewClientAction createNewClientAction;
    private final NewClientFormReadyGuard newClientFormReadyGuard;

    public Stage1StateMachineConfigExtended(
            ...
            NewClientFormReadyGuard newClientFormReadyGuard) {
        ...
        this.createNewClientAction = createNewClientAction;
        this.newClientFormReadyGuard = newClientFormReadyGuard;
    }
    ...
    public void configure(StateMachineTransitionConfigurer<Stage1State, Stage1Event> transitions) throws Exception {
        transitions
            ...
            // CLIENT_SELECTION -> CLIENT_SELECTION (при CLIENT_SELECTED з existing client guard)
            .withInternal()
            ...
            // CLIENT_SELECTION -> CLIENT_SELECTION (при CLIENT_SELECTED з new client form guard)
            // Цей перехід спрацьовує коли форма нового клієнта готова до створення
            .withInternal()
                .source(Stage1State.CLIENT_SELECTION)
                .event(Stage1Event.CLIENT_SELECTED)
                .guard(newClientFormReadyGuard)
                .action(createNewClientAction)

            .and()

            // CLIENT_SELECTION -> COMPLETED (при COMPLETE_CLIENT_SELECTION з client selected guard)
            .withExternal()
                .source(Stage1State.CLIENT_SELECTION)
            ...
            // CLIENT_SELECTION -> COMPLETED (при COMPLETE_CLIENT_SELECTION з new client form guard)
            // Альтернативний шлях завершення коли клієнт створений через форму
            .withExternal()
                .source(Stage1State.CLIENT_SELECTION)
                .target(Stage1State.COMPLETED)
                .event(Stage1Event.COMPLETE_CLIENT_SELECTION)
