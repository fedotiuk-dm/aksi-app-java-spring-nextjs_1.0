...
public class OrderValidationException extends RuntimeException {
     ...
     */
    public static OrderValidationException invalidStatusTransition(String fromStatus, String toStatus) {
        return new OrderValidationException(
            String.format("Неможливий перехід зі статусу %s до %s", fromStatus, toStatus)
        );
    }

    public static OrderValidationException invalidStatusTransition(OrderStatus fromStatus, OrderStatus toStatus) {
        return new OrderValidationException(
            String.format("Неможливий перехід зі статусу %s до %s", fromStatus.getDescription(), toStatus.getDescription())
        );
    }
    ...
    public static OrderValidationException cannotModifyOrder(String currentStatus) {
        return new OrderValidationException(
            "Неможливо змінити замовлення в статусі: " + currentStatus
        );
    }
    ...
    public static OrderValidationException cannotCompleteOrder(String reason) {
        return new OrderValidationException(
            "Неможливо завершити замовлення: " + reason
        );
    }
    ...
    public static OrderValidationException invalidInitialStatus(OrderStatus status) {
        return new OrderValidationException(
            "Неправильний початковий статус замовлення: " + status.getDescription() + ". Очікується: DRAFT"
        );
    }

    public static OrderValidationException cannotDeleteOrder(Long orderId, OrderStatus status) {
        return new OrderValidationException(
            String.format("Неможливо видалити замовлення %d в статусі %s", orderId, status.getDescription())
        );
    }

    public static OrderValidationException cannotModifyOrder(Long orderId, OrderStatus status) {
        return new OrderValidationException(
            String.format("Неможливо змінити замовлення %d в статусі %s", orderId, status.getDescription())
        );
    }
    ...
    public static OrderValidationException cannotChangeClientAfterProcessingStarted() {
        return new OrderValidationException("Неможливо змінити клієнта після початку обробки замовлення");
    }
    ...
    public static OrderValidationException deliveryDateTooEarly(UrgencyType urgencyType, LocalDateTime deliveryDate, LocalDateTime minDate) {
        return new OrderValidationException(
            String.format("Дата доставки %s занадто рання для терміновості %s. Мінімальна дата: %s",
    ...
    public static OrderValidationException discountTooHigh(BigDecimal discountAmount, BigDecimal maxDiscount) {
        return new OrderValidationException(
    ...
    public static OrderValidationException negativeItemsTotal(BigDecimal itemsTotal) {
        return new OrderValidationException(
            "Сума предметів не може бути від'ємною: " + itemsTotal
        );
    }
    ...
    public static OrderValidationException invalidCalculation(BigDecimal actual, BigDecimal expected) {
        return new OrderValidationException(
            String.format("Неправильний розрахунок суми. Фактична: %s, очікувана: %s", actual, expected)
        );
    }
}
