...
public class CartPricingServiceImpl implements CartPricingService {
  ...
  private static final List<ServiceCategoryType> NON_DISCOUNTABLE_CATEGORIES =
      List.of(ServiceCategoryType.LAUNDRY, ServiceCategoryType.IRONING, ServiceCategoryType.DYEING);
  ...
  public CartPricingInfo calculateCartPricing(Cart cart) {
    ...
    for (CartItem item : cart.getItems()) {
      CartItemPricingInfo itemPricing =
      ...
      if (isDiscountApplicable(
          item.getPriceListItem().getCategoryCode().name(), cart.getDiscountType())) {
  ...
  public CartItemPricingInfo calculateItemPricing(
    ...
    if (isDiscountApplicable(cartItem.getPriceListItem().getCategoryCode().name(), discountType)) {
      discountAmount =
          calculateDiscountAmount(subtotal + urgencyAmount, discountType, discountPercentage);
    }
    pricing.setDiscountAmount(discountAmount);
