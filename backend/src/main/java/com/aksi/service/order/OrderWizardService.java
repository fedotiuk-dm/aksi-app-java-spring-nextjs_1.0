...
/**
 * Service for order wizard functionality. Demonstrates usage of ServiceItemService methods for
 * order creation flow.
 */
@Service
...
public class OrderWizardService {
  ...
  /**
   * Get available items for selected service in order wizard This method demonstrates usage of
   * getAvailableItemsForService
   *
  ...
  public List<ServiceItemInfo> getAvailableItemsForOrder(UUID serviceId) {
    ...
    // - Active
    // - Available for order
    List<ServiceItemInfo> availableItems =
        serviceItemService.getAvailableItemsForService(serviceId);
  ...
  /**
   * Get all service-items for a service (including unavailable) This method demonstrates usage of
   * getServiceItemsByService Used in admin panel or for showing all options with availability
   * status
   *
