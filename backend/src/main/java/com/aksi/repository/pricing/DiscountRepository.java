...
public interface DiscountRepository extends JpaRepository<Discount, UUID> {
  ...
  @Query("SELECT d FROM Discount d WHERE d.active = true ORDER BY d.sortOrder, d.name")
  List<Discount> findAllActiveOrderedBySortOrder();
  ...
  List<Discount> findApplicableToCategoryCode(@Param("categoryCode") String categoryCode);

  /** Check if discount code exists */
  boolean existsByCode(String code);

  /** Check if discount code exists excluding current discount */
  @Query("SELECT COUNT(d) > 0 FROM Discount d WHERE d.code = :code AND d.id != :excludeId")
