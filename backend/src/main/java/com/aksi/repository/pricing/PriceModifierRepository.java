package com.aksi.repository.pricing;
...
@Repository
public interface PriceModifierRepository extends JpaRepository<PriceModifier, UUID> {

  /** Find modifier by code */
  Optional<PriceModifier> findByCode(String code);

  /** Find all active modifiers ordered by sort order */
  @Query("SELECT pm FROM PriceModifier pm WHERE pm.active = true ORDER BY pm.sortOrder, pm.name")
  List<PriceModifier> findAllActiveOrderedBySortOrder();

  /** Find active modifiers by category code */
  @Query(
      "SELECT pm FROM PriceModifier pm WHERE pm.active = true AND "
          + "(pm.categoryRestrictions IS EMPTY OR :categoryCode MEMBER OF pm.categoryRestrictions) "
          + "ORDER BY pm.sortOrder, pm.name")
  List<PriceModifier> findActiveByCategoryCode(@Param("categoryCode") String categoryCode);

  /** Find general modifiers (no category restrictions) */
  @Query(
      "SELECT pm FROM PriceModifier pm WHERE pm.active = true AND "
          + "pm.categoryRestrictions IS EMPTY ORDER BY pm.sortOrder, pm.name")
  List<PriceModifier> findGeneralModifiers();

  /** Check if modifier code exists */
  boolean existsByCode(String code);

  /** Check if modifier code exists excluding current modifier */
  @Query("SELECT COUNT(pm) > 0 FROM PriceModifier pm WHERE pm.code = :code AND pm.id != :excludeId")
