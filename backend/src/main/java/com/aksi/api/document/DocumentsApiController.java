...
/** HTTP Controller для DocumentsApi Відповідальність: тільки HTTP делегація до DocumentService */
@Controller
...
public class DocumentsApiController implements DocumentsApi {
  ...
  public ResponseEntity<DocumentResponse> updateDocumentStatus(
      UUID id, UpdateDocumentStatusRequest updateDocumentStatusRequest) {

    DocumentResponse response =
        documentService.updateDocumentStatus(id, updateDocumentStatusRequest);
    return ResponseEntity.ok(response);
  }
