...
public class PdfGenerationApiController implements PdfGenerationApi {
  ...
  @Override
  public ResponseEntity<DocumentResponse> generateReceiptPdf(
      UUID id, GeneratePdfRequest generatePdfRequest) {
    DocumentResponse response = pdfService.generateReceiptPdf(id, generatePdfRequest);
    return ResponseEntity.ok(response);
  }

  @Override
  public ResponseEntity<org.springframework.core.io.Resource> getReceiptPdf(
      UUID id, ReceiptTemplate template) {
