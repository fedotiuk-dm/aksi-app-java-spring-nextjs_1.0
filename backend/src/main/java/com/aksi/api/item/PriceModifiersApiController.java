...
public class PriceModifiersApiController implements PriceModifiersApi {
    ...
    @Override
    public ResponseEntity<PriceModifierResponse> createPriceModifier(CreatePriceModifierRequest request) {
    ...
    @Override
    public ResponseEntity<List<PriceModifierResponse>> getPriceModifiers(UUID categoryId, Boolean active) {
        List<PriceModifierResponse> response;
        if (Boolean.TRUE.equals(active)) {
            response = priceModifierService.getActivePriceModifiers();
        } else {
            response = priceModifierService.getPriceModifiers(org.springframework.data.domain.Pageable.unpaged()).getContent();
        }
        return ResponseEntity.ok(response);
    }

    @Override
    public ResponseEntity<PriceModifierResponse> updatePriceModifier(UUID id, UpdatePriceModifierRequest request) {
