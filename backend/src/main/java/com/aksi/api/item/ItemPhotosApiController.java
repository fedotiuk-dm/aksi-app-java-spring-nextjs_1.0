...
public class ItemPhotosApiController implements ItemPhotosApi {
    ...
    @Override
    public ResponseEntity<PhotoResponse> updatePhotoMetadata(UUID photoId, UpdatePhotoMetadataRequest request) {
        var response = itemPhotoService.updatePhotoMetadata(photoId, request);
        return ResponseEntity.ok(response);
    }

    @Override
    public ResponseEntity<PhotoResponse> uploadItemPhoto(UUID itemId, UUID itemId2, MultipartFile file, String description, String photoType, Boolean isMain) {
