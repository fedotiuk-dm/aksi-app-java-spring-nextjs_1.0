...
/**
 * HTTP Controller для ClientContactsApi
...
public class ClientContactsApiController implements ClientContactsApi {
    ...
    @Override
    public ResponseEntity<ClientContactsResponse> updateClientContacts(UUID uuid,
            UpdateClientContactsRequest updateClientContactsRequest) {
        ClientContactsResponse response = clientService.updateClientContacts(uuid, updateClientContactsRequest);
        return ResponseEntity.ok(response);
    }
}
