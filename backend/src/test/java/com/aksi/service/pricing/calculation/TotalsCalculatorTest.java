...
class TotalsCalculatorTest {
  ...
  void shouldCalculateTotalsForSingleItem() {
    // Given
    CalculatedItemPrice item = createCalculatedItem(
        15000, // subtotal
        0,     // urgency
        0,     // discount
  ...
  void shouldCalculateTotalsForMultipleItems() {
    // Given
    List<CalculatedItemPrice> items = Arrays.asList(
  ...
  void shouldCalculateTotalsWithUrgency() {
    ...
    when(priceCalculationService.getUrgencyPercentage(GlobalPriceModifiers.UrgencyTypeEnum.EXPRESS_48H))
        .thenReturn(50);
  ...
  void shouldCalculateTotalsWithDiscounts() {
    ...
    assertThat(result.getDiscountApplicableAmount()).isEqualTo(30000); // 100 + 200 (eligible items only)
  ...
  void shouldCalculateCustomDiscountPercentage() {
    // Given
    List<CalculatedItemPrice> items = List.of(
        createCalculatedItem(10000, 0, 2000, true, 8000)
    );
  ...
  private CalculatedItemPrice createCalculatedItem(
      int subtotal, int urgencyAmount, int discountAmount, 
      boolean discountEligible, int total) {
